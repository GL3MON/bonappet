...
const Login = () => {
    ...
    const handle_login = async () => {
        ...
        try {
            ...
            if (user_details.data.status == 200){
                localStorage.setItem('user_name', user_details.data.response.user_name);
                localStorage.setItem('user_mail', user_details.data.response.user_mail);
                localStorage.setItem('phone_number', user_details.data.response.phone_number);
                localStorage.setItem('address', user_details.data.response.address);
                nagivate("/")
            }else{
                setLoginFailed(true);
