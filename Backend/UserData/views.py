...
def partner_register(request):
    if request.method == 'POST':
        ...
        print(Users.objects.filter(email=employee_mail).email)
        if Users.objects.filter(email=employee_mail).exists():
            return JsonResponse({"error": "Registration failed. User already exists."}, status=200)
        partner = DeliveryPartners.objects.create_partner(request=request, name=name, phone_number=phone_no, vehicle_model=veh_model, license_number=license_no, vehicle_id=veh_id)
        partner.save()
